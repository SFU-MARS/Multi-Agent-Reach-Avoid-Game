...
# obs_lower_bound = np.array([-1.0, -1.0, -np.pi, -1.0, -1.0, -np.pi])
# obs_upper_bound= np.array([+1.0, +1.0, np.pi, 1.0, +1.0, np.pi])
# obs_space = spaces.Box(low=obs_lower_bound, high=obs_upper_bound, dtype=np.float32)
# action_space = spaces.Box(low=action_lower_bound, high=action_upper_bound, dtype=np.float32)

# model = PPO.load(trained_model, custom_objects={'observation_space':obs_space, 'action_space':action_space})
fac = ConfigFactoryTestAdversary()
config = fac.merge()
config.algo_config['training'] = False
config.output_dir = "/home/marslab/catkin_ws/src/turtlebot3_controller/scripts/test_resutls"
# rarl model
...
# rap model
# model_path = '/home/marslab/catkin_ws/src/turtlebot3_controller/scripts/training_results/dubin_rarl_game/rapgame/seed_42/10000000steps/model_latest.pt'
...
def vicon_data_callback():
    ...
    while not rospy.is_shutdown():
        ...
        control_defender = model.select_action(obs)
        ...
        if attacker_x >= 0.6 and attacker_x <= 0.8 and attacker_y >= 0.1 and attacker_y <= 0.30:
            ...
            defender_cmd.angular.z = control_defender[0]
...
if __name__ == "__main__":
    rospy.init_node("hj_controller_nn_1vs1")

    rospy.loginfo("Initialize the NN controller for 1vs1.")
    # rate = rospy.Rate(200)
